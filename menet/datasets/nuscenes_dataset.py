...
from .custom_3d import Custom3DDatasetCustom


@DATASETS.register_module(name="NuScenesDataset", force=True)
class NuScenesDatasetCustom(Custom3DDatasetCustom):
    r"""NuScenes Dataset.
    ...
    def get_data_info(self, index: int) -> Dict[str, Any]:
        ...
        if self.modality["use_lidar"]:
            data["pts_filename"] = info["lidar_path"]
        ...
        if self.modality["use_camera"]:
            ...
            data["lidar2camera"] = np.array(data["lidar2camera"])
            data["lidar2image"] = np.array(data["lidar2image"])
            data["camera2ego"] = np.array(data["camera2ego"])
            data["camera_intrinsics"] = np.array(data["camera_intrinsics"])
            data["camera2lidar"] = np.array(data["camera2lidar"])
            ...
            for i, box in enumerate(boxes):
                ...
                if np.sqrt(box.velocity[0]**2 + box.velocity[1]**2) > 0.2:
                    if name in [
                        ...
                        attr = NuScenesDatasetCustom.DefaultAttribute[name]
                else:
    ...
    def evaluate_map(self, results):
        ...
        assert self.map_classes is not None, "self.map_classes must be not None."
