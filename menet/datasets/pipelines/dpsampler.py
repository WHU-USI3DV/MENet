...
class MapEnhancedDataBaseSampler(DataBaseSamplerCustom):
    """Class for sampling data from the ground truth database.
    ...
    def __init__(
        ...
        super().__init__(info_path, data_root, rate, prepare, 
            sample_groups, classes, bbox_code_size, points_loader)
        self.xbound = xbound
        self.ybound = ybound
    ...
    def sample_class_v2(self, name, num, gt_info):
        ...
        if self.dataset.lower() == "nuscenes":
            ...
            # The map in lyft is a normlized RGB BEV image. The background is represented by (1,1,1).
            drivable_area_vechiles = (masks_bev.sum(0) != 3)
            drivable_area_others = drivable_area_vechiles
        else:
            raise NotImplementedError("Only surpport NuScenes and Lyft dataset now.")
